...
<<<<<<< HEAD
        ...
        <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
            <a href="{{ route('admin.categories.edit', $category->id) }}" class="inline-flex items-center justify-center rounded-md border border-transparent bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 sm:w-auto">
                Edit Category
=======
            <h1 class="text-xl font-semibold text-gray-900">Kategori: {{ $category->nom }}</h1>
        </div>
        <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
            <a href="{{ route('admin.categories.edit', $category->id) }}" class="inline-flex items-center justify-center rounded-md border border-transparent bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 sm:w-auto">
                Ubah Kategori
...
=======
        <h2 class="text-lg font-medium text-gray-900">Produk dalam kategori ini</h2>
>>>>>>> 4ec54b9e (update terbaru)
                        ...
                        <div class="p-5">
                            <div class="flex items-center">
                                <div class="flex-shrink-0">
                                    <img class="h-10 w-10 rounded-full" src="{{ image_url($product->image) }}" alt="">
                                </div>
                                <div class="ml-4">
                                    <div class="text-sm font-medium text-gray-900">
                                        <a href="{{ route('admin.products.show', $product->id) }}">{{ $product->nom }}</a>
                                    </div>
                                    <div class="text-sm text-gray-500">{{ rupiah($product->prix) }}</div>
                                </div>
                            </div>
                        </div>
                    </div>
...
=======
            <p class="mt-2 text-sm text-gray-500">Belum ada produk di kategori ini.</p>
>>>>>>> 4ec54b9e (update terbaru)
        @endif
    </div>
</div>
