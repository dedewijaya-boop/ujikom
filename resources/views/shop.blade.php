    ...
    <div class="bg-white shadow-sm">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900">Koleksi Kami</h1>
                    
                    <p class="mt-2 text-gray-600">Temukan berbagai dekorasi papan MDF unik dari Lasvara Decor</p>
                </div>
            ...
            <div class="mt-4">
                <div class="flex flex-wrap gap-2 items-center">
                    ...
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-emerald-100 text-emerald-800">
                        Kategori: {{ \App\Models\Category::find(request('category'))->nom ?? '' }}
                    ...
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-emerald-100 text-emerald-800">
                        @php
                            $priceRanges = [
                                '1' => 'Di bawah Rp. 50.000.00',
                                '2' => 'Rp. 30.000.00 - Rp. 20.000.00',
                                '3' => 'Rp. 15.000.00 - Rp. 10.000.00',
                                '4' => 'Di atas Rp. 100.000.00',
                            ];
                        @endphp
                        Harga: {{ $priceRanges[request('price')] ?? '' }}
                    ...
                    <a href="{{ route('shop') }}" class="text-sm text-emerald-600 hover:text-emerald-800">Hapus semua</a>
                </div>
            </div>
            @endif
        </div>
            ...
            <div class="md:w-64 pr-8 mb-8 md:mb-0">
                <form method="GET" action="{{ route('shop') }}">
                    <div class="bg-white p-4 rounded-lg shadow-sm sticky top-4">
                        <!-- Categories Filter -->
                        <div class="mb-6">
                            <h3 class="text-lg font-medium text-gray-900 mb-3">Kategori</h3>
                            <ul class="space-y-2">
                        ...
                        <!-- Price Filter -->
                        <div class="mb-6">
                            <h3 class="text-lg font-medium text-gray-900 mb-3">Harga</h3>
                            <div class="space-y-4">
                                <div class="flex items-center">
                                    <input type="radio" id="price-all" name="price" value="" 
                                           {{ !request('price') ? 'checked' : '' }}
                                           class="h-4 w-4 border-gray-300 text-emerald-600 focus:ring-emerald-500">
                                    <label for="price-all" class="ml-3 text-sm text-gray-600">Semua Harga</label>
                                </div>
                                <div class="flex items-center">
                                    <input type="radio" id="price-1" name="price" value="1" 
                                           {{ request('price') == '1' ? 'checked' : '' }}
                                           class="h-4 w-4 border-gray-300 text-emerald-600 focus:ring-emerald-500">
                                    <label for="price-1" class="ml-3 text-sm text-gray-600">Di bawah Rp. 50.000.00</label>
                                </div>
                                <div class="flex items-center">
                                    <input type="radio" id="price-2" name="price" value="2" 
                                           {{ request('price') == '2' ? 'checked' : '' }}
                                           class="h-4 w-4 border-gray-300 text-emerald-600 focus:ring-emerald-500">
                                    <label for="price-2" class="ml-3 text-sm text-gray-600">Rp.30.000.00 - Rp.15.000.00</label>
                                </div>
                                <div class="flex items-center">
                                    <input type="radio" id="price-3" name="price" value="3" 
                                           {{ request('price') == '3' ? 'checked' : '' }}
                                           class="h-4 w-4 border-gray-300 text-emerald-600 focus:ring-emerald-500">
                                    <label for="price-3" class="ml-3 text-sm text-gray-600">Rp. 15.000.00 - Rp. 10.000.00</label>
                                </div>
                                <div class="flex items-center">
                                    <input type="radio" id="price-4" name="price" value="4" 
                                           {{ request('price') == '4' ? 'checked' : '' }}
                                           class="h-4 w-4 border-gray-300 text-emerald-600 focus:ring-emerald-500">
                                    <label for="price-4" class="ml-3 text-sm text-gray-600">Di atas Rp.50.000.00</label>
                                </div>
                            </div>
                        </div>
...
@endfor
                                ...
                                <div class="mt-3 flex items-center justify-between">
                                    ...
                                    <form action="{{ route('cart.add', $product->id) }}" method="POST">
                                        @csrf
                                        <button type="submit" class="text-white bg-emerald-600 hover:bg-emerald-700 px-3 py-1 rounded-full text-sm transition-colors duration-300" title="Tambah ke Keranjang">
